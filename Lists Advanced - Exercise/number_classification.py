def positive_numbers(list_of_numbers):

    return ', '.join([positive for positive in list_of_numbers if int(positive) >= 0])


def negative_numbers(list_of_numbers):
    return ', '.join([negative for negative in list_of_numbers if int(negative) < 0])


def even_numbers(list_of_numbers):
    return ', '.join([even for even in list_of_numbers if int(even) % 2 == 0])


def odd_numbers(list_of_numbers):
    return ', '.join([odd for odd in list_of_numbers if int(odd) % 2 != 0])


numbers = [number for number in input().split(", ")]
print(f"Positive: {positive_numbers(numbers)}")
print(f"Negative: {negative_numbers(numbers)}")
print(f"Even: {even_numbers(numbers)}")
print(f"Odd: {odd_numbers(numbers)}")
